...
from predict import batch_predictions, make_prediction
...
def batch_predict(user_inputs : List[InputSchema]):
    predictions = batch_predictions([x.model_dump() for x in user_inputs])
    return [
        OutputSchema(predicted_price=round(prediction, 2))
